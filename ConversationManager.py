...
class ConversationManager:
    def __init__(self, promptname="assistantP.txt", conversation_history:list=[]):
    ...
    def convo_setup(self, *conversation_IDs):
        self.message(SYS, self.system_prompt)
    ...
    def message(self, role, content:str):
    ...
    def save(self, closing=False):
        ...
        if closing:
            ...
            if summary != None:
                self.message(SUMMARY, f"{summary}")
    ...
    def api_message_setup(self, user_input = None, api_data = None):
        self.message(SYS, self.system_prompt)
        if user_input != None:
            self.message(USER, str(user_input))
        if api_data != None:
            self.message(AGENT, str(api_data))
